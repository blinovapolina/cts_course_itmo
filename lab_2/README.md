...
| `RUN mkdir -p src` изменилось на `WORKDIR /usr/src/app` | Рабочая директория теперь фиксируется корректно (запускается эта директория для дальнейших слоев) |
...
Docker Compose позволяет удобно управлять многоконтейнерными приложениями. Однако ошибки в структуре compose-файла приводят к: 
- плохой безопасности, 
- неверной сетевой конфигурации, 
- трудности поддержки, 
- утечке чувствительных данных, 
- нестабильности при запуске. 

Даже при корректном Dockerfile неправильный compose-файл может разрушить изоляцию контейнеров или привести к непредсказуемому поведению. В данной работе мы рассмотрим примеры неправильного и правильного compose-файла и разберём, как исправления влияют на качество проекта.
...
![alt text](bad_docker_compose.jpg)
...
![alt text](good_docker_compose.jpg)
