...
|-------------------------------------|--------------|
| `network_mode: bridge` заменён на собственную сеть `isolated_net` с параметром `internal: true` | Изолировали контейнер на сететевомуровне. Повышена безопасность. |
